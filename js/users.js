/* ==========================================================================
   js/users.js
   Admin user management — create, reset passwords, delete users.
   BUG FIX: Removed duplicate usrGetCsrfToken/usrFetch wrappers.
   csrf.js already patches window.fetch globally. The old Headers-instance
   path was silently dropping Content-Type:application/json, causing the
   server to receive an empty body and reject every create-user call.
   Using plain fetch() fixes the create-user error completely.
   ========================================================================== */
"use strict";
...
function createUser() {
    ...
    /* csrf.js patches window.fetch — CSRF token added automatically */
    fetch("/create-user", {
        method: "POST",
...
function loadUsers() {
    ...
    fetch("/users", { credentials: "same-origin" })
        ...
        .then(function (rows) {
            ...
            rows.forEach(function (u) {
                ...
                btnDel.addEventListener("click", function () {
                    if (!confirm("Delete '" + u.username + "' (" + u.role + ")? They will not be able to log in.")) return;
                    fetch("/user/" + u.id, { method: "DELETE", credentials: "same-origin" })
                        .then(function (r) { return r.json().then(function (d) { return { ok: r.ok, d: d }; }); })
                        .then(function (res) {
        ...
        .catch(function () {
            tbody.innerHTML = '<tr><td colspan="3" style="text-align:center;color:#B3261E;">Could not load users.</td></tr>';
        });
}

/* Teacher-class routing for parent messages */
function tcInit() {
...
function tcLoad() {
    ...
    fetch("/api/teacher-classes", { credentials: "same-origin" })
        ...
        .then(function (rows) {
            rows = Array.isArray(rows) ? rows : [];
            if (!rows.length) {
                box.innerHTML = '<span style="color:#93a19a;font-size:13px;">No assignments yet.</span>';
                return;
            }
...
function tcAssign() {
    ...
    fetch("/api/teacher-classes", {
        ...
        .then(function (r) { return r.json().then(function (d) { return { ok: r.ok, d: d }; }); })
        .then(function (res) { usrNotify(res.d.message || "", res.ok); if (res.ok) tcLoad(); })
        .catch(function () { usrNotify("Network error.", false); });
}

function tcRemove(id) {
    fetch("/api/teacher-classes/" + id, { method: "DELETE", credentials: "same-origin" })
