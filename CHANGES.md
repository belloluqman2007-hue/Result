...
Report: **the attendance Mark Register and the Report disagreed about who is in a class.** `GET /attendance/class` found pupils through `students.class_name`, `GET /attendance/report` found them through `attendance.class_name`, and `attFillStudentPick()` on the attendance page compared `s.class_name === cls` with no status filter at all. As soon as a pupil was moved to another class — or a class was renamed — after attendance had already been taken, the same picked class could list pupils in the Report that the Mark Register never showed, and the history picker offered pupils the register hides.

### One shared "same class" rule

| File | What happened |
|---|---|
| `server.js` | NEW `attNormClass()` / `attSameClass()` / `attIsActiveStatus()` + `ATT_ACTIVE_ONLY_SQL` just above the attendance routes: ONE definition of "this is the same class" (`LOWER(TRIM(...))` on both sides, exactly like the register already did) and ONE definition of "this pupil is active" (a blank/missing status counts as active, the way every student page already reads it). |
| `server.js` — `GET /attendance/class` (Mark Register) | FIX: the roster is **every pupil whose CURRENT `students.class_name` is the picked class** — full stop. The day's saved mark is attached by `student_id + att_date` ONLY and is never filtered by the class name written on it, so a pupil who moved into the class (or whose class was renamed) after that day was marked now shows up **with** the status already given instead of staying invisible while the Report keeps listing them. Also hardened: one row per pupil is guaranteed (a database that predates `uniq_student_day` can't double-list anybody), a blank status counts as active, and a 3-step `ER_BAD_FIELD_ERROR` fallback ladder means a database still warming up its `students.status` / `attendance.marked_by` columns lists the class instead of returning an empty register. NEW response fields (additive): `class_name`, `marked_by`, `marked_class`, `marked_elsewhere`. |
| `server.js` — `GET /attendance/report` (Report) | FIX: history is **still** reported from `attendance.class_name` — the class each day was really taken in — so no saved row moves class and no total is recalculated. Two things changed: the class name is matched with the SAME `LOWER(TRIM(...))` rule as the register (a padded or differently-cased name can no longer pull the two tabs apart), and `JOIN students` became `LEFT JOIN students` with a display-name fallback, so marks for a pupil who has since been deleted from the students table still report instead of silently dropping out of the class totals. NEW response fields (additive): `current_class`, `current_status`, `moved`, `removed`, `inactive`; the counts are now real numbers instead of DECIMAL strings. |
| `server.js` — `GET /attendance/summary` | FIX: the "attendance already taken for this date" banner counts with the same class rule, so it can no longer say 0 marked while the register underneath loads saved marks. Read-only. |
| `server.js` — `POST /attendance/save` | **Untouched, byte for byte.** Rows still insert with the class the teacher picked and update on `(student_id, att_date)`; no class name is ever rewritten, so old marks stay with the class they were taken in. |
| `js/attendance.js` (and the root `attendance.js` copy) | NEW `attNormClass()` / `attSameClass()` / `attIsActiveStudent()` / `attRegisterRoster()` / `attReportRowNote()` — the client twin of the server rule. FIX `attFillStudentPick()`: it fetched `/students` with no status filter and used a plain `s.class_name === cls`, so it offered pupils the Mark Register never lists (withdrawn/graduated) and missed pupils it does list (a class name saved with a different case or one extra space failed the strict compare). It now offers **exactly the register's pupils, in the register's order**, with a "No active pupil in this class" line when the class is empty. An empty Mark Register now also explains itself ("If this class was renamed, or its pupils moved to another class, the Report tab still shows the attendance that was saved for them") instead of only saying "No students in this class yet." Register rows say where a loaded mark came from ("saved mark taken in JSS 1A"); report rows explain every pupil they show that today's register does not ("now in JSS2A" · "withdrawn" · "graduated" · "no longer on the student list - saved marks kept") plus one summary line under the table; the student-history header shows the pupil's CURRENT class. |
| `sw.js` | Cache v67. |

### Verified

Ran the **real** route handlers from `server.js` (stubbed `./db` → in-memory SQLite fixture) and the **real** client files inside a jsdom copy of `attendance.html`: **38 handler/client checks + 22 DOM checks passed for BOTH client copies** (82 checks, no failures), plus the extracted SQL re-run directly against the fixture. The fixture covers every way the two lists could drift apart: a pupil who moved OUT of the class, a pupil who moved IN (their mark still saved under the pre-rename name `JSS 1A`), a class name stored padded (`" JSS1A "`), a withdrawn pupil, a graduated pupil, a blank status, and a pupil deleted from `students`. Result: the Mark Register lists exactly the 4 pupils currently active in `JSS1A` and pre-selects the moved-in pupil's saved LATE mark; the Report keeps all 7 historical pupils with byte-identical counts (2/0/0/2 → 100%, 1/1/0/2 → 50%, …) and flags the 4 that are no longer in today's register; the picker offers exactly the register's 4 pupils. Phase 2 repeats the whole run for the pre-rename name `JSS 1A`: the register is honestly empty (with the new explanation), the picker agrees, and the Report still returns that saved LATE day flagged "now in JSS1A".

No attendance row was inserted, updated, moved or deleted by any of this. Result calculations, grading, positions, report card generation, printing, fees and every other staff/portal query: completely untouched.
